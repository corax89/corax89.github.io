...
function buildSwitch(){
	const name = replacementTextInput.value.trim();
	if(newIco)
		var img = base64ToByteArray(newIco);
	else
		var img = base64ToByteArray(ico);
	// Validate input
	if (!name) {
...
${gameLoopCode}
Game.initSensorInput();Game.init();document.addEventListener('DOMContentLoaded', () => {Game.enableTouchInput = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || window.matchMedia('(pointer: coarse)').matches;});
${customScript}
