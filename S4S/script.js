    ...
    <xml>
        ...
        <category name="${Blockly.Msg['OBJECTS']}" colour="340">
            ...
            <block type="object_distance"></block>
                        ...
                        <block type="create_pivot_joint"></block>
                        <block type="create_slide_joint"></block>
                        <block type="create_gear_joint"></block>
                        <block type="create_chain"></block>
                        <block type="remove_joint"></block>
                </category>
...
function togglePhysicsShapes(checkbox) {
        draw_physics_shapes = checkbox.checked;
}
...
`;
  ...
  // FIX: Sync the "view bounding box" checkbox state with draw_bounding_box
  // before starting the game. Without this, the user has to toggle the
  // checkbox multiple times after each run to make it take effect.
  var bbCheckbox = document.getElementById('objectsToggleBox');
  if (bbCheckbox) {
    draw_bounding_box = bbCheckbox.checked;
  }
  // Sync "view physics shapes" checkbox too.
  var psCheckbox = document.getElementById('physicsShapesToggle');
  if (psCheckbox) {
    draw_physics_shapes = psCheckbox.checked;
  }
